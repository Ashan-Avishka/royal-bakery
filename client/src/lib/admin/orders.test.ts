...
describe("getAdminOrder", () => {
  it("returns null for a 404 (well-formed id, order not found)", async () => {
    ...
    await expect(getAdminOrder("token", "missing-order")).resolves.toBeNull();
  });
  ...
  it("returns null for a 400 (malformed id) instead of throwing", async () => {
    ...
    await expect(getAdminOrder("token", "not-a-real-id")).resolves.toBeNull();
  });

  it("rethrows other errors", async () => {
    mocks.api.mockRejectedValue(new ApiError(500, "Server error"));

    await expect(getAdminOrder("token", "order-1")).rejects.toThrow("Server error");
  });
});
